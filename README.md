...
导出前记得 导出配置
1.0.30版本更新如下
修复了暗夜模式下宿主机状态和docker容器组件没有金色边框的问题
更新了版本号显示

- **日历组件视图精简**: 现在仅保留三种视图
  - `当天日历 (day)`：单格完整展示公历、星期、农历日期与干支年
  - `公历+农历+节假日 (month-lunar)`：展示农历、节气、节日与纪念日
  - `公历+节假日+纪念日 (month-memorial)`：仅展示重要节日/纪念日（隐藏普通农历与节气）
- **当天视图布局重写**: 采用左右分栏，左侧 1/3 显示超大日期，右侧展示年月、农历、星期与干支年，保证单格也能完整显示。
- **节日显示优先级优化**: 优先显示公历节日→农历节日→法定节假日→纪念日→节气→普通农历；统一移除名称尾部“节”。
- **尺寸调整持久化修复**: 修复了拖拽调整后“调整尺寸”不与桌面联动的问题，更新设备布局写入逻辑，确保持久化。
- **测试修复**: 解决 `GridPanel` 测试环境中 `window is not defined` 的问题。

**版本**:

- 内置版本号更新为 `v1.0.24`，保持与仓库标签同步。
