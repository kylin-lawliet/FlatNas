...
- **新增/优化**:
  - **日历月视图**:
    - 日期副标题优先显示二十四节气与节假日；无则回退显示农历（日为“初一”时显示月份名）。
    - 节气/节日当天只显示名称，不再显示数字天。
    - 自动移除节日名称尾部“节”字（如“圣诞节”→“圣诞”）。
    - 顶部上下月导航按钮居中，避免与右上角按钮遮挡。
  - **版本检测**: 更新内置版本号为 `v1.0.23`，保持与仓库标签同步。
  - **细节优化**: 提升月视图可读性，缩减空白、增大字号。
- **近期新增 (v1.0.15)**:
  - **多用户模式**: 新增多用户模式支持。
  - **侧边栏组件**: 添加侧边栏组件。
